...
function once_get_current_theme_name()
{
    $theme = '';
    try {
...
function once_get_theme_option_name()
{
    $theme = once_get_current_theme_name();
    return $theme !== '' ? ('theme:' . $theme) : null;
}

function once_get_theme_settings_from_db()
{
    $name = once_get_theme_option_name();
    if (!$name) {
        return [];
    }
...
function once_get_backup_option_prefix()
{
    $theme = once_get_current_theme_name();
    return $theme !== '' ? ('once_theme_backup:' . $theme . ':') : null;
}

function once_create_db_backup_entry(array $settings)
{
    $prefix = once_get_backup_option_prefix();
    if (!$prefix) {
...
function once_list_db_backups($limit = 20)
{
    $prefix = once_get_backup_option_prefix();
    if (!$prefix) {
        return [];
...
function once_get_db_backup_json($name)
{
    $prefix = once_get_backup_option_prefix();
    $name = (string)$name;
...
function once_delete_db_backup($name)
{
    $prefix = once_get_backup_option_prefix();
    $name = (string)$name;
    if (!$prefix || strpos($name, $prefix) !== 0) {
...
function once_export_theme_settings_json()
{
    $theme = once_get_current_theme_name();
    $settings = once_get_theme_settings_from_db();
...
function once_clear_output_buffers()
{
...
function once_sanitize_imported_settings(array $settings)
{
    unset($settings['_'], $settings['once_backup_data'], $settings['once_backup_file']);

    foreach (array_keys($settings) as $k) {
        if (is_string($k) && str_starts_with($k, 'once_backup_')) {
            unset($settings[$k]);
        }
    }
...
function once_save_theme_settings_to_db(array $settings)
{
    $name = once_get_theme_option_name();
    if (!$name) {
        throw new Exception('Unknown theme');
...
function once_handle_theme_backup_request()
{
    if (!defined('__TYPECHO_ADMIN__')) {
        return;
    }

    if (empty($_GET['once_backup'])) {
        return;
    }

    $action = (string)$_GET['once_backup'];
    $user = User::alloc();
    ...
    if ($action === 'save_db') {
        ...
        try {
            $settings = once_get_theme_settings_from_db();
            $key = once_create_db_backup_entry($settings);
    ...
    if ($action === 'restore_db') {
        once_clear_output_buffers();
        $key = (string)($_GET['key'] ?? '');
        $json = once_get_db_backup_json($key);
        if (!$json) {
        ...
        try {
            $settings = once_sanitize_imported_settings($settings);
            once_save_theme_settings_to_db($settings);
    ...
    if ($action === 'export') {
        once_clear_output_buffers();
        $json = once_export_theme_settings_json();
        $file = 'theme-' . once_get_current_theme_name() . '-backup-' . date('Ymd-His') . '.json';
    ...
    if ($action === 'import') {
        ...
        $raw = (string)($_POST['once_backup_data'] ?? '');
        $raw = trim($raw);
        if ($raw === '') {
...
function once_render_theme_backup_section()
{
    if (!defined('__TYPECHO_ADMIN__')) {
        return;
    }
    ...
    $security = Security::alloc();
    //$exportUrl = $security->getAdminUrl('options-theme.php?once_backup=export');
    $importUrl = $security->getAdminUrl('options-theme.php?once_backup=import');
    $saveDbUrl = $security->getAdminUrl('options-theme.php?once_backup=save_db');
    $json = once_export_theme_settings_json() ?: '{}';
    //$safeJson = htmlspecialchars($json, ENT_QUOTES, 'UTF-8');
    $backups = once_list_db_backups(20);
    ...
    if (empty($backups)) {
        ...
        foreach ($backups as $item) {
            ...
            $restoreUrl = $security->getAdminUrl('options-theme.php?once_backup=restore_db&key=' . urlencode($name));
            $deleteUrl = $security->getAdminUrl('options-theme.php?once_backup=delete_db&key=' . urlencode($name));
        ...
        if (restoreBtn && importEl) {
            restoreBtn.addEventListener("click", async function(){
                ...
                try {
                    ...
                    form.append("once_backup_data", raw);
...
once_handle_theme_backup_request();